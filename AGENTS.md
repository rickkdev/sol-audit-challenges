...
- Keep sanitization helpers in `src/sol_audit_challenges/sanitize.py`; sanitizer reports may include replacement labels/counts but must not print sensitive replacement `find` strings from private config. Use sanitizer `rename` rules for anonymized file paths that must continue matching sanitized imports.
...
- Keep complete fake end-to-end fixtures under `examples/fixtures/` with matching fake manifests/reports in `examples/`; committed fake oracle examples should use a safe suffix such as `.oracle.example.json` because real `*.private.json` and `*.oracle.json` files are ignored.
- Keep operator security-boundary guidance in `docs/operating-model.md`; private candidate intake and verification workflow details belong in `docs/private-research-workflow.md`.
